...
app.post("/theatre/registershow", async (c) => {
  ...
  console.log(body);
  showData.total_seats = parseInt(showData.total_seats);
  showData.booked_seats = parseInt(showData.booked_seats);
  showData.ticket_price = parseInt(showData.ticket_price);
...
app.get("/theatre/details/:id", async (c) => {
  const prisma = new PrismaClient({
    datasourceUrl: c.env.DATABASE_URL,
  }).$extends(withAccelerate());

  const theatreId = parseInt(c.req.param("id"));
  ...
  try {
    ...
    if (shows.length === 0) {
      return c.json({
        message: `No shows found for theatre with ID ${theatreId}.`,
        status: false,
      });
    }

    return c.json({ message: "Shows found.", status: true, data: shows });
  } catch (error) {
